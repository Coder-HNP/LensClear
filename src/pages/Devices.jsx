...
import { unlinkDeviceFromUser, renameDevice } from "../utils/firestoreAPI";
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 space-y-4">
                                    {loading ? (
                                        ...
                                        devices.map((device) => {
                                            const rawStatus = (device.status || "").toString().toLowerCase();
                                            const isOnline =
                                                rawStatus === "online" ||
                                                rawStatus === "active" ||
                                                rawStatus === "running";
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="flex items-center gap-4 w-full">
                                                        <div
                                                            className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${iconClasses}`}
                                                        >
                                                            <Wifi size={24} />
                                                        </div>
                                                        <div className="flex-1">
                                                            ...
                                                            <p className="text-xs text-gray-400 font-mono mt-0.5">
                                                                Auth ID: {device.authToken ? (
                                                                    <span className="bg-gray-100 px-1 rounded select-all text-gray-600 font-medium">
                                                                        {device.authToken}
                                                                    </span>
                                                                ) : (
                                                            ...
                                                            <div className="flex items-center gap-3 mt-1">
                                                                ...
                                                                {device.lastOnline && (
                                                                    <span className="text-xs text-gray-400">
                                                                        Last seen:{" "}
                                                                        {device.lastOnline?.toDate?.().toLocaleDateString?.() ||
                                                                            (device.lastOnline instanceof Date
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                        <div className="flex items-center justify-between mb-6">
                                            {(() => {
                                                const rawStatus = (selectedDevice?.status || '').toString().toLowerCase();
                                                const isOnline = rawStatus === 'online' || rawStatus === 'active' || rawStatus === 'running';
                                                const isOffline = rawStatus === 'offline';
                                                const statusLabel = isOnline
                                                    ? 'Online'
                                                    : isOffline
                                                ...
                                                return (
                                                    <>
                                                        <div className="flex items-center gap-4">
                                                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${iconClasses}`}>
                                                                <Wifi size={24} />
                                                            </div>
                                                            <div>
                                                        ...
                                                        <span className={`text-sm font-medium px-3 py-1 rounded-full ${badgeClasses}`}>
                                                            {statusLabel}
                                                        </span>
                                                    </>
                                                );
